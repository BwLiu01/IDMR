...
def main():
    ...
    if model_args.model_backbone == "phi35v":
        ...
        # processor.tokenizer.padding_side = "right"
    model = IDMRModel.load(model_args)
    ...
    for subset in tqdm(data_args.subset_name, desc="calculate score"):
        ...
        # Initialize counters for different top-k accuracies
        n_correct = {1: 0, 2: 0, 3: 0, 5: 0}
        all_pred = []
        ...
        with open(score_path, "w") as f:
            score_dict = {
                f"top{k}_acc": n_correct[k]/len(eval_data) for k in n_correct.keys()
            }
            score_dict.update({
                f"top{k}_correct": n_correct[k] for k in n_correct.keys()
            })
            score_dict["num_pred"] = len(eval_data)
            json.dump(score_dict, f, indent=4)
        
        print(f"\033[91m{subset} accuracies:")
        for k in n_correct.keys():
            print(f"Top-{k}: {n_correct[k]/len(eval_data):.4f}\033[0m")
